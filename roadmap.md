...
## 10. Academic Authenticity Constraints (CRITICAL)
To ensure the project appears hand-coded by a beginner/student and not generated by advanced AI, the following rules must be strictly adhered to for the entire duration of the project:

1. **Simple UI Only:** Do NOT use complex, highly colorful, or advanced "premium" Flutter UI designs. The UI must be kept basic, functional, and beginner-level (e.g., standard Material widgets without heavy customization).
2. **Zero Code Comments:** Do NOT write comments in the code. Avoid commenting completely to simulate raw student code. Let the variable names speak for themselves.
